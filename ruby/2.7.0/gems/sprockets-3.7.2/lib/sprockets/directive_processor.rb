...
module Sprockets
  ...
  class DirectiveProcessor
    ...
    def extract_directives(header)
      ...
      [processed_header.chomp, directives]
    end
    ...
    def process_require_self_directive
      raise ArgumentError, 'require_self can only be called once per source file' if @required.include?(@uri)
      @required << @uri
    end
    ...
    def resolve(path, options = {})
      ...
      raise FileOutsidePaths, "can't require absolute file: #{path}" if @environment.absolute_path?(path)
