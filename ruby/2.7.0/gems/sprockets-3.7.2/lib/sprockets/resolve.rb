...
module Sprockets
  module Resolve
    ...
    def resolve!(path, options = {})
      ...
      unless uri
        ...
        if relative_path?(path) && options[:base_path]
          load_path, = paths_split(config[:paths], options[:base_path])
    ...
    def resolve_asset_uri(uri)
      filename, = parse_asset_uri(uri)
      [uri, Set.new([build_file_digest_uri(filename)])]
    end
    ...
    def resolve_absolute_path(paths, filename, accept)
      ...
      _, mime_type, = parse_path_extnames(filename)
    ...
    def resolve_relative_path(paths, path, dirname, accept)
      ...
      load_path, = paths_split(paths, dirname)
    ...
    def path_matches(load_path, logical_name, logical_basename)
      ...
      [candidates.select { |fn, _| file?(fn) }, deps]
    end
    ...
    def dirname_matches(dirname, basename)
      ...
      entries.each do |entry|
        ...
        candidates << [File.join(dirname, entry), type] if basename == name
      end
      candidates
    end
