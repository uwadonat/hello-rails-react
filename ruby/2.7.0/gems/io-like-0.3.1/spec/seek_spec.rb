...
describe 'IO::Like#seek' do
  ...
  it 'moves the read position relative to the current position with SEEK_CUR' do
    -> { @iowrapper.seek(-1) }.should raise_error(Errno::EINVAL)
  ...
  it 'can handle any numerical argument without breaking' do
    ...
    -> { @iowrapper.seek(2**128) }.should raise_error(RangeError)
  end

  it 'raises IOError on closed stream' do
    -> { IOSpecs.closed_file.seek(0) }.should raise_error(IOError)
  end
