...
describe 'IO::Like#readpartial' do
  before :each do
    @rd, @wr = IO.pipe
    @iowrapper_rd = IOWrapper.open(@rd)
    @iowrapper_wr = IOWrapper.open(@wr)
  ...
  it 'raises IOError on closed stream' do
    -> { IOSpecs.closed_file.readpartial(10) }.should raise_error(IOError)

    @iowrapper_rd.close
    -> { @iowrapper_rd.readpartial(10) }.should raise_error(IOError)
  end
  ...
  it 'raises EOFError on EOF' do
    ...
    -> { @iowrapper_rd.readpartial(10) }.should raise_error(EOFError)
  end
  ...
  it 'discards the existing buffer content upon error' do
    ...
    @iowrapper_wr.close
    @wr.close
    -> { @iowrapper_rd.readpartial(1, buffer) }.should raise_error(EOFError)
  ...
  it 'raises IOError if the stream is closed' do
    @iowrapper_wr.close
    @wr.close
    -> { @iowrapper_rd.readpartial(1) }.should raise_error(IOError)
  end

  it 'raises ArgumentError if the negative argument is provided' do
    -> { @iowrapper_rd.readpartial(-1) }.should raise_error(ArgumentError)
  end
