...
describe 'IO::Like#print' do
  before :each do
    @old_record_separator = $OUTPUT_RECORD_SEPARATOR
  ...
  it 'appends $\\ to the output' do
    ...
    File.read(@filename).should == "#{data}#{$OUTPUT_RECORD_SEPARATOR}"
  end
  ...
  it 'writes $, between arguments' do
    ...
    File.read(@filename).should == "#{data1}#{$OUTPUT_FIELD_SEPARATOR}#{data2}"
  end
  ...
  it 'does not check if the file is writable if writing zero bytes' do
    -> { IOSpecs.readable_iowrapper.print('') }.should_not raise_error
  end
  ...
  it 'does not raise IOError on closed stream if writing zero bytes' do
    -> { IOSpecs.closed_file.print('') }.should_not raise_error
  end

  it 'raises IOError on closed stream if writing more than zero bytes' do
    -> { IOSpecs.closed_file.print('hello') }.should raise_error(IOError)
  end
end
