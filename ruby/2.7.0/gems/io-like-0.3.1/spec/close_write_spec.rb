...
describe 'IO::Like#close_write' do
  ...
  it 'closes the write end of a duplex I/O stream' do
    ...
    -> { @iowrapper.write('attempt to write') }.should raise_error(IOError)
  end

  it 'raises an IOError on subsequent invocations' do
    @iowrapper.close_write
    -> { @iowrapper.close_write }.should raise_error(IOError)
  end

  it 'allows subsequent invocation of close' do
    @iowrapper.close_write
    -> { @iowrapper.close }.should_not raise_error
  end
  ...
  it 'raises IOError on closed stream' do
    -> { IOSpecs.closed_file.close_write }.should raise_error(IOError)
  end
end
