...
describe 'IO::Like#read' do
  ...
  it 'reads all data available before a SystemCallError is raised' do
    ...
    -> { @iowrapper.read }.should raise_error(SystemCallError)
    @iowrapper.read.should == @contents
  end
  ...
  it 'raises IOError on write-only stream' do
    -> { IOSpecs.writable_iowrapper.read }.should raise_error(IOError)
  end

  it 'raises IOError on closed stream' do
    -> { IOSpecs.closed_file.read }.should raise_error(IOError)
  end
end
