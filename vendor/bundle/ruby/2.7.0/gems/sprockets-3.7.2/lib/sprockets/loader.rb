...
module Sprockets
  ...
  module Loader
    ...
    def load(uri)
      ...
      if unloaded.params.key?(:id)
        unless asset = asset_from_cache(unloaded.asset_key)
          ...
          @logger.warn "Sprockets load error: Tried to find #{uri}, but latest was id #{asset[:id]}" if asset[:id] != id
        end
      else
    ...
    def load_from_unloaded(unloaded)
      raise FileNotFound, "could not find file: #{unloaded.filename}" unless file?(unloaded.filename)

      load_path, logical_path = paths_split(config[:paths], unloaded.filename)

      unless load_path
        raise FileOutsidePaths, "#{unloaded.filename} is no longer under a load path: #{paths.join(', ')}"
      end
      ...
      if processors.any?
        ...
        metadata = {
          ...
          length: stat(unloaded.filename).size,
      ...
      asset[:mtime] = metadata[:dependencies].map do |u|
        if u.start_with?('file-digest:')
          s = stat(parse_file_digest_uri(u))
          s ? s.mtime.to_i : nil
        end
      end.compact.max
      asset[:mtime] ||= stat(unloaded.filename).mtime.to_i
