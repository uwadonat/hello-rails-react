...
module Sprockets
  ...
  module Server
    ...
    def call(env)
      ...
      return method_not_allowed_response unless %w[GET HEAD].include?(env['REQUEST_METHOD'])
      ...
      # 2.x just ignored it.
      fingerprint = nil if asset && parse_asset_uri(asset.uri)[1][:pipeline] == 'self'
      ...
      if_none_match = env['HTTP_IF_NONE_MATCH'][/^"(\w+)"$/, 1] if env['HTTP_IF_NONE_MATCH']
    ...
    def headers(env, asset, length)
      ...
      if type = asset.content_type
        ...
        type += "; charset=#{asset.charset}" if type.start_with?('text/') && asset.charset
