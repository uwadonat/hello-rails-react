...
module Racc
  ...
  class States
    ...
    def state_transition_table
      @state_transition_table ||= StateTransitionTable.generate(dfa)
    end
    ...
    def generate_states(state)
      ...
      table.each do |sym, core|
        ...
        next unless state.ident == dest.ident and state.closure.size == 1
        raise CompileError,
              format('Infinite recursion: state %d, with rule %d',
    ...
    def lookahead
      ...
      edge = []
      gotos.each do |goto|
        goto.to_state.goto_table.each do |t, _st|
          if t.terminal?
      ...
      gotos.each do |goto|
        goto.symbol.heads.each do |ptr|
          ...
          addrel lookback, st.rruleid(ptr.rule), goto if st.conflict?
    ...
    def digraph(map, relation)
      ...
      index.each_index do |i|
        traverse i, index, vertices, map, relation if !(index[i]) and relation[i]
      end
    end
    ...
    def traverse(i, index, vertices, map, relation)
      ...
      if rp = relation[i]
        rp.each do |proci|
          traverse proci, index, vertices, map, relation unless index[proci]
    ...
    def resolve_rr(state, r)
      r.each do |item|
        item.each_la(@symboltable) do |t|
          act = state.action[t]
          if act
            raise "racc: fatal: #{act.class} in action table" unless act.is_a?(Reduce)
    ...
    def pack(state)
      ...
      act.each do |_t, a|
        arr[a.ruleid] += 1 if a.is_a?(Reduce)
      end
      i = arr.max
      ...
      if s
        r = @actions.reduce(s)
        if !state.defact or state.defact == r
          act.delete_if { |_t, a| a == r }
          state.defact = r
        end
      else
    ...
    def check_useless
      ...
      @symboltable.nt_base.upto(@symboltable.nt_max - 1) do |n|
        @symboltable[n].useless = true unless used[n]
      end
    end
  ...
  class State
    ...
    def check_la(la_rules)
      ...
      unless r.empty?
        @conflict = true if !s.empty? or r.size > 1
      end
      s.compact!
      @stokens = s
      @rrules = r
  ...
  class SRconflict
    ...
    def to_s
      format('state %d: S/R conflict rule %d reduce and shift %s',
  ...
  class RRconflict
    ...
    def to_s
      format('state %d: R/R conflict with rule %d and %d on %s',
