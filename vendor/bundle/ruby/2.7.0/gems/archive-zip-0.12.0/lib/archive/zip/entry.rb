                  ...
                  module Entry
                    ...
                    def self.expand_path(zip_path)
                      ...
                      source.each do |e|
                        ...
                        if e == '..' && !(result.last.nil? || result.last == '..')
                          result.pop
                        else
                          result.push(e)
                        end
                    ...
                    def self.from_file(file_path, options = {})
                      ...
                      unless options[:compression_codec].nil?
                        entry.compression_codec = if options[:compression_codec].is_a?(Proc)
                                                    options[:compression_codec][entry].new
                                                  else
                                                    options[:compression_codec].new
                                                  end
                      end
                      unless options[:encryption_codec].nil?
                        entry.encryption_codec = if options[:encryption_codec].is_a?(Proc)
                                                   options[:encryption_codec][entry].new
                                                 else
                                                   options[:encryption_codec].new
                                                 end
                      end
                    ...
                    def self.parse(io)
                      ...
                      unless IOExtensions.read_exactly(io, 4) == LFH_SIGNATURE
                      ...
                      entry = if cfr.zip_path[-1..-1] == '/'
                                # This is a directory entry.
                                Entry::Directory.new(expanded_path, io_window)
                              elsif (cfr.external_file_attributes >> 16) & 0o770000 == 0o120000
                                # This is a symlink entry.
                                Entry::Symlink.new(expanded_path, io_window)
                              else
                                # Anything else is a file entry.
                                Entry::File.new(expanded_path, io_window)
                              end
                      ...
                      # Unix-compatible.
                      entry.mode = cfr.external_file_attributes >> 16 if cfr.made_by_version & 0xFF00 == 0x0300
                    ...
                    def self.parse_local_file_record(io, expected_compressed_size = nil)
                      ...
                      if lfr.general_purpose_flags & FLAG_DATA_DESCRIPTOR_FOLLOWS > 0
                        saved_pos = io.pos
                        ...
                        if possible_signature == DD_SIGNATURE
                          lfr.crc32,
                          lfr.compressed_size,
                          lfr.uncompressed_size =
                    ...
                    def zip_path=(zip_path)
                      ...
                      raise ArgumentError, 'zip path expands to empty string' if @zip_path.empty?
                    end
                    ...
                    def dump_local_file_record(io, local_file_record_position)
                      ...
                      if encryption_codec.class == Codec::TraditionalEncryption
                        # HACK:
                      ...
                      general_purpose_flags |= FLAG_DATA_DESCRIPTOR_FOLLOWS if need_trailing_data_descriptor
                      ...
                      if encryption_codec.version_needed_to_extract > version_needed_to_extract
                      ...
                      if need_trailing_data_descriptor
                    ...
                    def central_extra_field_data
                      @central_extra_field_data = @extra_fields.collect(&:dump_central).join
                    end

                    def dummy
                      # Add fields for time data if available.
                      unless mtime.nil? && atime.nil?
                      ...
                      unless uid.nil? || gid.nil? || mtime.nil? || atime.nil?
                    ...
                    def local_extra_field_data
                      @local_extra_field_data = @extra_fields.collect(&:dump_local).join
                    end
...
module Archive; class Zip; module Entry
                             ...
                             class Directory
                               ...
                               def extract(options = {})
                                 ...
                                 file_path = options.key?(:file_path) ?
                                                       options[:file_path].to_s :
                                                       @zip_path
                                 restore_permissions = options.key?(:permissions) ?
                                                       options[:permissions] :
                                                       false
                                 restore_ownerships = options.key?(:ownerships) ?
                                                       options[:ownerships] :
                                                       false
                                 restore_times = options.key?(:times) ?
                                                       options[:times] :
                                                       false
                               ...
                               def link_target
                                 ...
                                 encryption_codec.decryptor(raw_data, password) do |decryptor|
                                   compression_codec.decompressor(decryptor) do |decompressor|
                                     ...
                                     begin
                                       unless expected_data_descriptor.nil?
...
module Archive; class Zip; module Entry
                             ...
                             class File
                               ...
                               # file.
                               def mode=(mode)
                                 super(0o100000 | (mode & 0o7777))
                               end

                               # Sets the decryption password.
                               def password=(password)
                                 @file_data = nil unless @raw_data.nil?
                                 @password = password
                               end
                               ...
                               def file_data
                                 ...
                                 if raw_data.nil?
                                   simulated_raw_data = if @file_path.nil?
                                                          BinaryStringIO.new
                                                        else
                               ...
                               def file_data=(file_data)
                                 ...
                                 unless @file_data.respond_to?(:data_descriptor)
                               ...
                               def extract(options = {})
                                 ...
                                 ::File.open(file_path, 'wb') do |f|
                                   while buffer = file_data.read(4096)
                                     f.write(buffer)
                                   end
                                 end
