...
describe 'IO::Like#sysseek on a file' do
  ...
  it 'raises an error when called after buffered reads' do
    @iowrapper.readline
    -> { @iowrapper.sysseek(-5, IO::SEEK_CUR) }.should raise_error(IOError)
  end
  ...
  it 'warns if called immediately after a buffered IO#write' do
    begin
      ...
      wrapper.write('abcde')
      -> { wrapper.sysseek(10) }.should complain(/sysseek/)
    ensure
      wrapper.close
  ...
  it 'moves the read position relative to the end with SEEK_END' do
    ...
    -> { @iowrapper.sysread(1) }.should raise_error(EOFError)
  ...
  it 'can handle any numerical argument without breaking and can seek past EOF' do
    ...
    @iowrapper.sysseek(2**32).should == 4_294_967_296
    -> { @iowrapper.sysseek(2**128) }.should raise_error(RangeError)
  end

  it 'raises IOError on closed stream' do
    -> { IOSpecs.closed_file.sysseek(0) }.should raise_error(IOError)
  end
end
