...
describe 'IO::Like#sysread on a file' do
  before :each do
    @filename = tmp('IO_Like___sysread_file') + $PROCESS_ID.to_s
  ...
  it 'raises IOError when called immediately after a buffered IO#read' do
    @iowrapper.read(15)
    -> { @iowrapper.sysread(5) }.should raise_error(IOError)
  end

  it 'does not raise IOError if called after IO#read followed by IO#write' do
    @iowrapper.read(5)
    @iowrapper.write('abcde')
    -> { @iowrapper.sysread(5) }.should_not raise_error(IOError)
  end
  ...
  it 'raises IOError on write-only stream' do
    -> { IOSpecs.writable_iowrapper.sysread(5) }.should raise_error(IOError)
  end

  it 'raises IOError on closed stream' do
    -> { IOSpecs.closed_file.sysread(5) }.should raise_error(IOError)
  end
end
